...
//Testing Birefringence
	...
	//Checking content on RayStep first
//	cout << "RayStep x's dimension: " << RayStep[0][0].size()<< endl; 
//	cout << "RayStep z's dimension: " << RayStep[0][1].size() << endl; 

//	double time_diff_birefringence = birefringence->Time_Diff_TwoRays(RayStep[0][0], RayStep[0][1], settings1);
//	cout << "Two Ray Time Difference: " << time_diff_birefringence << endl;

//	TVector3 p_e1 = birefringence->Get_p_e1();
//	TVector3 p_e2 = birefringence->Get_p_e2();

//	double Pol_birefringence[2][3];
	
//	for(int i=0; i<3; i++){
//		Pol_birefringence[0][i] = p_e1[i]; 
//		Pol_birefringence[1][i] = p_e2[i]; 
//	}
//cout << "Gets here" << endl;

//Finish Testing Birefringence
...
//						if ( n + T_shift < stations[i].strings[j].antennas[k].Nnew[ray_sol_cnt] ){
//							V_forfft_birefringence[1][n] = V_forfft[ n + T_shift];   
//						}	
//						else{
//							V_forfft_birefringence[1][n] = 0.;	
//						}
//					    } 
...
//SPLIT POWER: Take this vector (unitary already) and dot it with p_e1 and p_e2 at step 0 to get its projections


//					    double split_factor = birefringence->Power_split_factor(Pol_vector, bire_ray_cnt, settings1);
					    int T_shift_bire = int(time_diff_birefringence/dT_forfft);  
...
//					    for (int n=0; n<stations[i].strings[j].antennas[k].Nnew[ray_sol_cnt]; n++){
//					           V_forfft[n] = V_forfft[n] * split_factor;
//cout << "Does it enter the loop? " << n << endl;
//					    }
...
//MADE CHANGES HERE!!					
//					    for ( int n = 0; n < stations[i].strings[j].antennas[k].Nnew[ray_sol_cnt]; n++ ){
//					    	for (int bire_ray_cnt = 0; bire_ray_cnt < settings1->BIREFRINGENCE + 1;  bire_ray_cnt++){
//							V_forfft[n] += V_forfft_bire[bire_ray_cnt][n];
//						}
//					    }
					    birefringence->Two_rays_interference(V_forfft, V_forfft_bire[0], V_forfft_bire[1], stations[i].strings[j].antennas[k].Nnew[ray_sol_cnt], max_bire_ray_cnt, settings1);
...
//                                            std::cout << std::endl;
                                            ...
                                            // }
                                            // std::cout << std::endl;                                              
//NEXT: What does this do? Need to decide to do this before or after adding the waveforms of the two rays.  
                                            Tools::SincInterpolation(stations[i].strings[j].antennas[k].Nnew[ray_sol_cnt], T_forfft, V_forfft, settings1->NFOUR / 2, T_forint, volts_forint);
 
                                            // volts_forint = V_forfft;
                                            // T_forint = T_forfft;
//                                            cout << "gain_ch_no = " << gain_ch_no << endl;
//                                            cout << "Final waveform v(t) = " << endl;
                                            for (int n = 0; n < sizeof(volts_forint) / sizeof(volts_forint[0]); n++) {
//                                              std::cout << settings1->ARBITRARY_EVENT_ATTENUATION*volts_forint[n]*2. / (double)(stations[i].strings[j].antennas[k].Nnew[ray_sol_cnt]) << ", ";
                                            }
//                                            std::cout << std::endl;                                                
//                                            cout << "iiiiiiiii" << endl;
                                            //End debugging
...
void Report::Select_Wave_Convlv_Exchange(Settings *settings1, Trigger *trigger, Detector *detector, int signalbin, vector <double> &V, int *noise_ID, int ID, int StationIndex) {
    ...
    // cout << "Select_wave_convlv_1" << endl;
...
//    int BINSIZE = detector->stations[StationIndex].NFOUR/2;
    ...
    for (int bin=0; bin<BINSIZE; bin++) {   //BINSIZE should be NFOUR/2
        ...
        if ( settings1->NOISE_CHANNEL_MODE==0) {
            // cout << "v_noise_timedomain = " << trigger->v_noise_timedomain[ noise_ID[ (int)( bin_value / settings1->DATA_BIN_SIZE) ] ][ (int)( bin_value % settings1->DATA_BIN_SIZE ) ] << endl;
...
void Report::Select_Wave_Convlv_Exchange(Settings *settings1, Trigger *trigger, Detector *detector, int signalbin1, int signalbin2, vector <double> &V1, vector <double> &V2, int *noise_ID, int ID, int StationIndex) {
    ...
    int BINSIZE = settings1->NFOUR/2;
    // cout << "Select_wave_convlv_2" << endl;
//    int BINSIZE = detector->stations[StationIndex].NFOUR/2;
    int bin_value;
...
void Report::Select_Wave_Convlv_Exchange(Settings *settings1, Trigger *trigger, Detector *detector, int signalbin0, int signalbin1, int signalbin2, vector <double> &V0, vector <double> &V1, vector <double> &V2, int *noise_ID, int ID, int StationIndex) {
    ...
    int BINSIZE = settings1->NFOUR/2;
    // cout << "Select_wave_convlv_3" << endl;
//    int BINSIZE = detector->stations[StationIndex].NFOUR/2;
    int bin_value;
    int signal_dbin = signalbin2 - signalbin1;
